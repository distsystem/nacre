...
def materialize_branch(settings: config_module.BranchMaterializerSettings) -> None:
    ...
    for remote in settings.fetch:
        git_ops.run_git(settings.repo, "fetch", remote)

    checked_out_worktree = git_ops.worktree_for_branch(settings.repo, settings.target_branch)
    ...
    with tempfile.TemporaryDirectory(prefix="materialize-branch-") as temp_dir:
        ...
        try:
            ...
            final_commit = git_ops.run_git(temp_path, "rev-parse", "HEAD")
        finally:
    ...
    if checked_out_worktree is not None:
        ...
        git_ops.run_git(settings.repo, "branch", "-f", settings.target_branch, final_commit)
